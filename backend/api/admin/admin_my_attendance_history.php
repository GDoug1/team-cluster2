...
if ($attendanceUserColumn === null || $attendancePrimaryKey === null || $currentUserId <= 0) {
    echo json_encode([]);
    exit;
}
...
$attendanceJoin = '';
$orderBy = 'COALESCE(al.updated_at, al.created_at) DESC';
...
if ($hasLegacyAttendance) {
    ...
    $orderBy = 'COALESCE(al.time_in_at, al.updated_at) DESC, al.id DESC';
    ...
    $orderBy = 'COALESCE(al.attendance_date, tl.time_in, al.updated_at) DESC, al.attendance_id DESC';
}
